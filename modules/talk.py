...
def cmds(event):
    ...
    ид[id пользователя]: информация о пользователе вк
    ...
    аи: сгенерировать изображение по запросу
    гпт [запрос]: ChatGPT 3.5
    выбери[1 или 2]: выбирает то что вы укажете
    
    Осу:
    о линк [ваш никнейм в осу]: привязывает ваш профиль в осу к вашему профилю вк
    о ник: проверка ника который привязан к вашему профилю вк
    о л/ласт: показывает последнюю сыгранную карту за 24 часа
    о с/стата : показывает статистику профиля
    о м osu/mania : выбор режима
...
def choose(event):
    ...
    b = [first_str,second_str]
    r = random.choice(b)
    event.message_send (f'''Я думаю лучше выбрать: {r}''')
